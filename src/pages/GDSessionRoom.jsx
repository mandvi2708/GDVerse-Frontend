...
function GDSessionRoom() {
      ...
      recognition.onresult = (event) => {
        ...
        if (transcript.trim()) {
          socket.emit('transcript-update', {
            roomId: inviteLink,
            sender: 'User ' + socket.id.slice(0, 4),
            text: transcript
          });
        }
      };
  ...
  }, [isTranscribing, audioEnabled, inviteLink]);
  ...
  const leaveMeeting = async () => {
    if (window.confirm("Do you want to generate Minutes of the Meeting before leaving?")) {
      await handleGenerateMOM();
    }
  ...
  const handleGenerateMOM = async () => {
    ...
    try {
      ...
      if (sessionData && sessionData._id) {
        const genResponse = await fetch(`${import.meta.env.VITE_BACKEND_URL}/api/ai/generate-mom/${sessionData._id}`, {
          method: 'POST'
        });
      ...
      <div className="flex-1 flex flex-col">
        ...
        <div className="bg-gray-800 p-4 flex items-center justify-center space-x-6">
          ...
          <button
            ...
            className={`p-3 rounded-full ${isTranscribing ? 'bg-yellow-500 hover:bg-yellow-600' : 'bg-gray-700 hover:bg-gray-600'} transition-all`}
            title={isTranscribing ? 'Stop AI Transcription' : 'Start AI Transcription'}
          >
            <svg xmlns="http://www.w3.org/2000/svg" className="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 11a7 7 0 01-7 7m0 0a7 7 0 01-7-7m7 7v4m0 0H8m4 0h4m-4-8a3 3 0 01-3-3V5a3 3 0 116 0v6a3 3 0 01-3 3z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 5v2m0 4v2m0 4v2M5 5v2m0 4v2m0 4v2m5-14v2m0 4v2m0 4v2" />
            </svg>
          </button>
          ...
          <button
            ...
            className={`p-3 rounded-full ${isGeneratingMom ? 'bg-gray-600' : 'bg-purple-600 hover:bg-purple-700'} transition-all`}
            title="Generate AI Minutes of Meeting"
          >
            {isGeneratingMom ? (
      ...
      {showMomModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm">
          <div className="bg-gray-800 rounded-2xl w-full max-w-2xl max-h-[80vh] flex flex-col shadow-2xl border border-gray-700">
            ...
            <div className="p-6 overflow-auto flex-1 text-gray-300 prose prose-invert">
